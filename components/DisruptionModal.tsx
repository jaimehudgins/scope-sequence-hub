...
import { CascadePreview as CascadePreviewType, NonInstructionalDay } from '@/types';
import {
  buildCascadePreview,
...
const DISRUPTION_TYPES: { value: NonInstructionalDay['type']; label: string; icon: string }[] = [
...
export default function DisruptionModal() {
  ...
  const [disruptionType, setDisruptionType] = useState<NonInstructionalDay['type']>('snow');
      ...
      if (isMultiDay) {
        ...
        for (const courseId of courseIds) {
          ...
          const preview = buildMultiDayCascadePreview(
            lessons, courseId, targetDate, rangeEnd, course, nonInstructionalDays
          );
        ...
        return { shifts: allShifts, overflowLessons: allOverflow, affectedCourseIds: allAffected };
      } else {
        // Single day
        return buildMultiCourseCascadePreview(
          lessons, courseIds, targetDate, courses, nonInstructionalDays
        );
      }
    }

    if (disruptionModal.mode === 'insert-school-created' && schoolCreatedCourseId) {
      ...
      return buildCascadePreview(
        lessons, schoolCreatedCourseId, targetDate, course, nonInstructionalDays
      );
    }

    return null;
  }, [disruptionModal, lessons, courses, nonInstructionalDays, scope, selectedCourseIds, endDate, schoolCreatedCourseId]);
  ...
  const handleCancelDay = () => {
    ...
    const rangeEnd = endDate || targetDate;
    const description = rangeEnd > targetDate
    ...
    // Execute cascade
    const updatedLessons = executeCascade(lessons, cascadePreview, selectedForRemoval);
    setLessons(updatedLessons);

    const shiftCount = cascadePreview.shifts.filter((s) => s.newDate !== null && !selectedForRemoval.has(s.lesson.id)).length;
    const removedCount = selectedForRemoval.size;
    addToast(`🚫 ${label}: ${shiftCount} lesson${shiftCount !== 1 ? 's' : ''} rescheduled${removedCount > 0 ? `, ${removedCount} moved to Lesson Bank` : ''}`);
  ...
  const handleInsertSchoolCreated = () => {
    ...
    const newLesson = {
      ...
      description: schoolCreatedDescription.trim() || `School-created lesson: ${schoolCreatedTitle.trim()}`,
    ...
    const shiftCount = cascadePreview?.shifts.filter((s) => s.newDate !== null && !selectedForRemoval.has(s.lesson.id)).length || 0;
    addToast(`🏫 "${schoolCreatedTitle.trim()}" added${shiftCount > 0 ? `, ${shiftCount} lesson${shiftCount !== 1 ? 's' : ''} rescheduled` : ''}`);
    ...
    >
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-border flex items-center justify-between flex-shrink-0">
          <div>
            <h2 className="font-serif text-[20px] font-semibold text-text">
              {isCancelMode ? '🚫 Cancel Day' : '🏫 Add School-Created Lesson'}
            </h2>
            <p className="text-[13px] text-text-muted mt-[2px]">{formattedDate}</p>
          </div>
          <button
            onClick={closeDisruptionModal}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-5">
              ...
              <div>
                ...
                {endDate && endDate > targetDate && (
                  <p className="text-[11px] text-text-muted mt-1">
                    Range: {formattedDate} through {format(stringToDate(endDate), 'EEEE, MMMM d, yyyy')}
                  </p>
                )}
              </div>
                  ...
                  <div className="flex flex-wrap gap-[6px]">
                    {Object.values(courses).map((course) => (
                      <button
                        ...
                        style={selectedCourseIds.has(course.id) ? { backgroundColor: course.color, borderColor: course.color } : {}}
                      >
                        <span className="w-2 h-2 rounded-full" style={{ backgroundColor: selectedCourseIds.has(course.id) ? 'white' : course.color }} />
                        {course.name}
                      </button>
                    ))}
                  </div>
                ...
                <div className="flex flex-wrap gap-[6px]">
                  {Object.values(courses).map((course) => (
                    <button
                      ...
                      style={schoolCreatedCourseId === course.id ? { backgroundColor: course.color, borderColor: course.color } : {}}
                    >
                      <span className="w-2 h-2 rounded-full" style={{ backgroundColor: schoolCreatedCourseId === course.id ? 'white' : course.color }} />
                      {course.name}
                    </button>
                  ))}
                </div>
          ...
          {/* Cascade Preview */}
          {cascadePreview && cascadePreview.shifts.length > 0 && (
            <div>
              <label className="text-[12px] font-semibold text-text-muted uppercase tracking-[0.8px] mb-[6px] block">
                Schedule Impact
              </label>
