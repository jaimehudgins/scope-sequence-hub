...
export default function CourseSchedulePopover({ courseId, anchorRect, onClose }: CourseSchedulePopoverProps) {
  ...
  const toggleDay = (dayValue: number) => {
    ...
    const dayNames = newDays.map((d) => DAY_LABELS.find((dl) => dl.value === d)?.label).join(', ');
  ...
  return (
    <div ref={popoverRef} style={style} className="bg-surface rounded-xl border border-border shadow-lg p-4 min-w-[220px] animate-[fade-in_0.15s_ease]">
      <div className="text-[11px] font-semibold uppercase tracking-[0.8px] text-text-muted mb-1">
        Meeting Schedule
      </div>
      <div className="text-[13px] font-medium text-text mb-3" style={{ color: course.color }}>
        {course.name}
      </div>
        ...
        {DAY_LABELS.map(({ value, label }) => {
          ...
          return (
            <button
              ...
              `}
              style={isActive ? { backgroundColor: course.color, borderColor: course.color } : {}}
            >
              {label}
            </button>
          );
